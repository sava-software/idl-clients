...
public record PoolState(PublicKey _address,
  ...
  public static PoolState readChecked(final PublicKey _address, final byte[] _data) {
    return readChecked(_address, _data, 0);
  }

  public static PoolState readChecked(final PublicKey _address, final byte[] _data, final int _offset) {
    if (_data == null || _data.length == 0) {
      return null;
    }
    if (!DISCRIMINATOR.equals(_data, _offset)) {
      throw new IllegalArgumentException("Not a PoolState account.");
    }
    return read(_address, _data, _offset);
  }

  public static final BiFunction<PublicKey, byte[], PoolState> FACTORY = PoolState::readChecked;
  ...
  public static PoolState read(final PublicKey _address, final byte[] _data, final int _offset) {
    ...
    i += 16;
    final var sqrtPriceX64 = getUInt128LE(_data, i);
    i += 16;
    ...
    i += 2;
    final var feeGrowthGlobal0X64 = getUInt128LE(_data, i);
    i += 16;
    final var feeGrowthGlobal1X64 = getUInt128LE(_data, i);
    i += 16;
    final var protocolFeesToken0 = getInt64LE(_data, i);
    i += 8;
    final var protocolFeesToken1 = getInt64LE(_data, i);
    i += 8;
    final var swapInAmountToken0 = getUInt128LE(_data, i);
    i += 16;
    final var swapOutAmountToken1 = getUInt128LE(_data, i);
    i += 16;
    final var swapInAmountToken1 = getUInt128LE(_data, i);
    i += 16;
    final var swapOutAmountToken0 = getUInt128LE(_data, i);
    i += 16;
