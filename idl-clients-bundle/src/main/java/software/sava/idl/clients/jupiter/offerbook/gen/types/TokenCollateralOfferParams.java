// @generated by sava-software idl-src-gen — DO NOT EDIT
package software.sava.idl.clients.jupiter.offerbook.gen.types;

import software.sava.idl.clients.core.gen.SerDe;

import static software.sava.core.encoding.ByteUtil.getInt32LE;
import static software.sava.core.encoding.ByteUtil.getInt64LE;
import static software.sava.core.encoding.ByteUtil.putInt32LE;
import static software.sava.core.encoding.ByteUtil.putInt64LE;

/// @param principalAmount: u64
/// @param collateralAmount: u64
/// @param apy: u32
/// @param duration: u32
/// @param expiry: u32
/// @param minFillAmount: u64
public record TokenCollateralOfferParams(long principalAmount,
                                         long collateralAmount,
                                         long apy,
                                         long duration,
                                         long expiry,
                                         boolean allowPartialFill,
                                         long minFillAmount) implements SerDe {

  public static final int BYTES = 37;

  public static final int PRINCIPAL_AMOUNT_OFFSET = 0;
  public static final int COLLATERAL_AMOUNT_OFFSET = 8;
  public static final int APY_OFFSET = 16;
  public static final int DURATION_OFFSET = 20;
  public static final int EXPIRY_OFFSET = 24;
  public static final int ALLOW_PARTIAL_FILL_OFFSET = 28;
  public static final int MIN_FILL_AMOUNT_OFFSET = 29;

  public static TokenCollateralOfferParams read(final byte[] _data, final int _offset) {
    if (_data == null || _data.length == 0) {
      return null;
    }
    int i = _offset;
    final var principalAmount = getInt64LE(_data, i);
    i += 8;
    final var collateralAmount = getInt64LE(_data, i);
    i += 8;
    final var apy = Integer.toUnsignedLong(getInt32LE(_data, i));
    i += 4;
    final var duration = Integer.toUnsignedLong(getInt32LE(_data, i));
    i += 4;
    final var expiry = Integer.toUnsignedLong(getInt32LE(_data, i));
    i += 4;
    final var allowPartialFill = _data[i] == 1;
    ++i;
    final var minFillAmount = getInt64LE(_data, i);
    return new TokenCollateralOfferParams(principalAmount,
                                          collateralAmount,
                                          apy,
                                          duration,
                                          expiry,
                                          allowPartialFill,
                                          minFillAmount);
  }

  @Override
  public int write(final byte[] _data, final int _offset) {
    int i = _offset;
    putInt64LE(_data, i, principalAmount);
    i += 8;
    putInt64LE(_data, i, collateralAmount);
    i += 8;
    putInt32LE(_data, i, (int) apy);
    i += 4;
    putInt32LE(_data, i, (int) duration);
    i += 4;
    putInt32LE(_data, i, (int) expiry);
    i += 4;
    _data[i] = (byte) (allowPartialFill ? 1 : 0);
    ++i;
    putInt64LE(_data, i, minFillAmount);
    i += 8;
    return i - _offset;
  }

  @Override
  public int l() {
    return BYTES;
  }
}
