...
/// @param numOfObsoleteDepositReserves: u8
/// @param hasDebt: u8
/// @param borrowingDisabled: u8
/// @param autodeleverageTargetLtvPct: u8
/// @param lowestReserveDepositMaxLtvPct: u8
/// @param numOfObsoleteBorrowReserves: u8
/// @param ownershipTransferState: u8
/// @param highestBorrowFactorPct: u64
/// @param autodeleverageMarginCallStartedTimestamp: u64
/// @param padding3: u64[]
public record Obligation(PublicKey _address,
  ...
  public static Obligation readChecked(final PublicKey _address, final byte[] _data) {
    return readChecked(_address, _data, 0);
  }

  public static Obligation readChecked(final PublicKey _address, final byte[] _data, final int _offset) {
    if (_data == null || _data.length == 0) {
      return null;
    }
    if (!DISCRIMINATOR.equals(_data, _offset)) {
      throw new IllegalArgumentException("Not a Obligation account.");
    }
    return read(_address, _data, _offset);
  }

  public static final BiFunction<PublicKey, byte[], Obligation> FACTORY = Obligation::readChecked;
  ...
  public static Obligation read(final PublicKey _address, final byte[] _data, final int _offset) {
    ...
    i += 8;
    final var depositedValueSf = getUInt128LE(_data, i);
    i += 16;
    final var borrows = new ObligationLiquidity[5];
    i += SerDeUtil.readArray(borrows, ObligationLiquidity::read, _data, i);
    final var borrowFactorAdjustedDebtValueSf = getUInt128LE(_data, i);
    i += 16;
    final var borrowedAssetsMarketValueSf = getUInt128LE(_data, i);
    i += 16;
    final var allowedBorrowValueSf = getUInt128LE(_data, i);
    i += 16;
    final var unhealthyBorrowValueSf = getUInt128LE(_data, i);
    i += 16;
