...
/// @param vaultBump: u8 Derivation bump of the vault PDA this transaction belongs to.
...
public record VaultTransaction(PublicKey _address,
  ...
  public static VaultTransaction readChecked(final PublicKey _address, final byte[] _data) {
    return readChecked(_address, _data, 0);
  }

  public static VaultTransaction readChecked(final PublicKey _address, final byte[] _data, final int _offset) {
    if (_data == null || _data.length == 0) {
      return null;
    }
    if (!DISCRIMINATOR.equals(_data, _offset)) {
      throw new IllegalArgumentException("Not a VaultTransaction account.");
    }
    return read(_address, _data, _offset);
  }

  public static final BiFunction<PublicKey, byte[], VaultTransaction> FACTORY = VaultTransaction::readChecked;
