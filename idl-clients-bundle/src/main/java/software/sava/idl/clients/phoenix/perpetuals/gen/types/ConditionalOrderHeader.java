...
public record ConditionalOrderHeader(PublicKey _address,
  ...
  public static ConditionalOrderHeader readChecked(final PublicKey _address, final byte[] _data) {
    return readChecked(_address, _data, 0);
  }

  public static ConditionalOrderHeader readChecked(final PublicKey _address, final byte[] _data, final int _offset) {
    if (_data == null || _data.length == 0) {
      return null;
    }
    if (!DISCRIMINATOR.equals(_data, _offset)) {
      throw new IllegalArgumentException("Not a ConditionalOrderHeader account.");
    }
    return read(_address, _data, _offset);
  }

  public static final BiFunction<PublicKey, byte[], ConditionalOrderHeader> FACTORY = ConditionalOrderHeader::readChecked;
