...
public record Tick(PublicKey _address,
  ...
  public static Filter createTotalIdsFilter(final long totalIds) {
    final byte[] _data = new byte[4];
    putInt32LE(_data, 0, (int) totalIds);
  ...
  public static Filter createLiquidationBranchIdFilter(final long liquidationBranchId) {
    final byte[] _data = new byte[4];
    putInt32LE(_data, 0, (int) liquidationBranchId);
  ...
  public static Tick readChecked(final PublicKey _address, final byte[] _data) {
    return readChecked(_address, _data, 0);
  }

  public static Tick readChecked(final PublicKey _address, final byte[] _data, final int _offset) {
    if (_data == null || _data.length == 0) {
      return null;
    }
    if (!DISCRIMINATOR.equals(_data, _offset)) {
      throw new IllegalArgumentException("Not a Tick account.");
    }
    return read(_address, _data, _offset);
  }

  public static final BiFunction<PublicKey, byte[], Tick> FACTORY = Tick::readChecked;
  ...
  public static Tick read(final PublicKey _address, final byte[] _data, final int _offset) {
    ...
    ++i;
    final var liquidationBranchId = Integer.toUnsignedLong(getInt32LE(_data, i));
    i += 4;
  ...
  public int write(final byte[] _data, final int _offset) {
    ...
    ++i;
    putInt32LE(_data, i, (int) liquidationBranchId);
    i += 4;
