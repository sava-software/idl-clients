...
package software.sava.idl.clients.oracles.pyth.pro.receiver.gen.types;
...
public record TwapUpdate(PublicKey _address, Discriminator discriminator, PublicKey writeAuthority, TwapPrice twap) implements SerDe {
  ...
  public static TwapUpdate readChecked(final PublicKey _address, final byte[] _data) {
    return readChecked(_address, _data, 0);
  }

  public static TwapUpdate readChecked(final PublicKey _address, final byte[] _data, final int _offset) {
    if (_data == null || _data.length == 0) {
      return null;
    }
    if (!DISCRIMINATOR.equals(_data, _offset)) {
      throw new IllegalArgumentException("Not a TwapUpdate account.");
    }
    return read(_address, _data, _offset);
  }

  public static final BiFunction<PublicKey, byte[], TwapUpdate> FACTORY = TwapUpdate::readChecked;
