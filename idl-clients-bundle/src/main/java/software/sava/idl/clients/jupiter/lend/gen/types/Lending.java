...
/// @param lendingId: u16
/// @param decimals: u8 _dev number of decimals for the fToken, same as ASSET
...
public record Lending(PublicKey _address,
  ...
  public static Lending readChecked(final PublicKey _address, final byte[] _data) {
    return readChecked(_address, _data, 0);
  }

  public static Lending readChecked(final PublicKey _address, final byte[] _data, final int _offset) {
    if (_data == null || _data.length == 0) {
      return null;
    }
    if (!DISCRIMINATOR.equals(_data, _offset)) {
      throw new IllegalArgumentException("Not a Lending account.");
    }
    return read(_address, _data, _offset);
  }

  public static final BiFunction<PublicKey, byte[], Lending> FACTORY = Lending::readChecked;
