...
public record OptionProposalMeta(PublicKey _address, Discriminator discriminator, PublicKey proposal, String[] optionDescriptions) implements SerDe {
  ...
  public static OptionProposalMeta readChecked(final PublicKey _address, final byte[] _data) {
    return readChecked(_address, _data, 0);
  }

  public static OptionProposalMeta readChecked(final PublicKey _address, final byte[] _data, final int _offset) {
    if (_data == null || _data.length == 0) {
      return null;
    }
    if (!DISCRIMINATOR.equals(_data, _offset)) {
      throw new IllegalArgumentException("Not a OptionProposalMeta account.");
    }
    return read(_address, _data, _offset);
  }

  public static final BiFunction<PublicKey, byte[], OptionProposalMeta> FACTORY = OptionProposalMeta::readChecked;
