...
public record Whirlpool(PublicKey _address,
  ...
  public static Whirlpool readChecked(final PublicKey _address, final byte[] _data) {
    return readChecked(_address, _data, 0);
  }

  public static Whirlpool readChecked(final PublicKey _address, final byte[] _data, final int _offset) {
    if (_data == null || _data.length == 0) {
      return null;
    }
    if (!DISCRIMINATOR.equals(_data, _offset)) {
      throw new IllegalArgumentException("Not a Whirlpool account.");
    }
    return read(_address, _data, _offset);
  }

  public static final BiFunction<PublicKey, byte[], Whirlpool> FACTORY = Whirlpool::readChecked;
  ...
  public static Whirlpool read(final PublicKey _address, final byte[] _data, final int _offset) {
    ...
    i += 16;
    final var sqrtPrice = getUInt128LE(_data, i);
    i += 16;
    ...
    i += 32;
    final var feeGrowthGlobalA = getUInt128LE(_data, i);
    i += 16;
    final var tokenMintB = readPubKey(_data, i);
    i += 32;
    final var tokenVaultB = readPubKey(_data, i);
    i += 32;
    final var feeGrowthGlobalB = getUInt128LE(_data, i);
    i += 16;
