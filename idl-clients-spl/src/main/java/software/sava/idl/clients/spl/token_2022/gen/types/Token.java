...
/// @param extensions The extensions activated on the token account.
public record Token(PublicKey _address,
  ...
  public static final int EXTENSIONS_OFFSET = 165;
  ...
  public static Token read(final PublicKey _address, final byte[] _data, final int _offset) {
    ...
    if (i < _data.length) {
      if (i + 1 > _data.length) {
        throw new IllegalArgumentException("A hidden prefix of 1 bytes at offset " + i + " runs past the " + _data.length + " byte buffer.");
      }
      if ((_data[i] & 0xFF) != 2) {
        throw new IllegalArgumentException("Expected the hidden prefix constant 2 at offset " + i + ", found " + (_data[i] & 0xFF) + ".");
      }
      i += 1;
  ...
  public int l() {
    return 32
         ...
         + 8
         + 36
         + (extensions == null ? 0 : 1 + SerDeUtil.lenArray(extensions));
  }
}
