...
below were worked down; every one of the 10 remaining rows is analyzed, and
falls into one of three groups, none of which is "not got to yet":

| Rows | Group | Status |
|---|---|---|
| 5 | Redundant zero short-circuits | equivalent — accepted, reasoned below |
| 3 | `authorizeStakeAccount*WithSeed` | **blocked on an idl-src-gen defect** (below) |
...
### Blocked: `authorizeStakeAccount*WithSeed` (3 rows)

`SPLClient.authorizeStakeAccountWithSeed` and
`authorizeStakeAccountCheckedWithSeed` throw `ArrayIndexOutOfBoundsException`
for **every** input. `AuthorizeWithSeedArgs.l()` and
`AuthorizeCheckedWithSeedArgs.l()` omit the 8-byte u64 length prefix that their
own `write()` emits ahead of the seed string, so the caller under-allocates by
exactly 8 bytes.

Those records are `@generated by sava-software idl-src-gen — DO NOT EDIT`, so
the fix belongs upstream, not here; `AuthorizeWithSeedParams` and
`AuthorizeCheckedWithSeedParams` carry the same defect but are currently unused.
`SystemProgram`'s seeded builders are unaffected — their `l()` includes the
prefix — so this is specific to the stake `*WithSeed*` types.

The tests encoding the intended contract are written and `@Disabled` with this
reason in `SPLClientTests`; re-enable them once idl-src-gen is fixed and the
sources regenerated.
