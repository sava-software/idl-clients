package software.sava.idl.clients.phoenix.dev.perpetuals.gen.types;

import software.sava.idl.clients.core.gen.SerDe;

/// Borsh payload for updating open interest cap for a perp asset.
///
public record UpdatePerpOpenInterestCapInstruction(Symbol perpAssetSymbol, BaseLots openInterestCap) implements SerDe {

  public static final int BYTES = 24;

  public static final int PERP_ASSET_SYMBOL_OFFSET = 0;
  public static final int OPEN_INTEREST_CAP_OFFSET = 16;

  public static UpdatePerpOpenInterestCapInstruction read(final byte[] _data, final int _offset) {
    if (_data == null || _data.length == 0) {
      return null;
    }
    int i = _offset;
    final var perpAssetSymbol = Symbol.read(_data, i);
    i += perpAssetSymbol.l();
    final var openInterestCap = BaseLots.read(_data, i);
    return new UpdatePerpOpenInterestCapInstruction(perpAssetSymbol, openInterestCap);
  }

  @Override
  public int write(final byte[] _data, final int _offset) {
    int i = _offset;
    i += perpAssetSymbol.write(_data, i);
    i += openInterestCap.write(_data, i);
    return i - _offset;
  }

  @Override
  public int l() {
    return BYTES;
  }
}
