...
* **spl:** Solana Attestation Service accounts decode at their real offsets. `Credential`, `Schema` and `Attestation` carry a one-byte discriminator (0/1/2) with fields starting at offset 1, not the eight-byte Anchor prefix previously assumed, so every `*_OFFSET` constant and any memcmp filter built from one changes. The prior layout matched none of the accounts on chain.
* **oracles:** the pyth receivers build `initialize` without an account discriminator. `Config` is an account reused as an instruction argument, and an argument is serialized fields-only; the builder emitted eight surplus bytes the program does not read. Instruction data is now 8 bytes shorter and `InitializeIxData` decodes a real instruction correctly. `Config.readFields`/`writeFields`/`lFields` are the argument forms; `read`/`write`/`l` remain the discriminator-prefixed account forms.
* `read(AccountInfo)` and `FACTORY` now throw IllegalArgumentException where they previously returned a record built from the wrong bytes. `FACTORY` is the sharper edge — sava-rpc applies it inside the JSON parse loop with no per-element guard, so one unexpected account fails the whole response rather than yielding one bad record the caller could skip. A size-only scan is where this bites: a `getProgramAccounts` filtered on `SIZE_FILTER` alone picks up any same-sized account of that program. To keep the previous lenient behaviour substitute `X::read` for `X.FACTORY`; to make the scan precise add `X.DISCRIMINATOR_FILTER`, which every account type emits.
