...
`agentsTemplateInSync` but no mutation suite. `:hardeningCertifyAll` (the root
manifest over both modules' receipts) and a local `fuzzAll` per module are
therefore release-checklist items here, not CI's.
...
84-added/76-removed delta, none of it local adaptation. Adopted the same way on
21.5.32 (marker `714041431f01`, from `f866084114e0`): three bullets changed. The
line-less-keys bullet grew the guarded two-preview `BaselinePrune` protocol, the
rule that a retained row stays authority whatever its note, line-less
identifiers for new evidence prose, and what a rebase's provenance stamp does
and does not claim; the consumer-notes bullet now prefers a `hardeningHelp`
pointer while forbidding a repository-wide prose migration on an upgrade; and
the invalid-outcomes bullet adds that a later clean run closes a non-recurring
invalid outcome and that an interrupted certification is retried per project.
None of it was local adaptation.
